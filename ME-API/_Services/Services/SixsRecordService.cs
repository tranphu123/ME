...
namespace ME_API._Services.Services
{
    public class SixsRecordService : ISixsRecordService
    {
        ...
        public SixsRecordService(IAuditRateDRepository auditRateDRepository, 
        IAuditRateMRepository auditRateMRepository, IAuditTypeRepository auditTypeMRepository,
        IMesAuditOrgRepository mesAuditOrg,IAuditTypeDRepository auditTypeDRepository,
        ...
        public async Task<object> GetAuditTypeByBranBySixs(string brand)
        {
            var auditType = await _auditTypeMRepository.FindAll(x => x.Brand.Trim() == brand && x.Audit_Kind.Trim() == "6S").ProjectTo<AuditTypeDto>(_configMapper).Distinct().OrderBy(x => x.Audit_Type_ID).ToListAsync();
            var data = from a in auditType
                    select new
                    {
                    audit_Type_ID =auditType.Where(y=> y.Audit_Kind ==a.Audit_Kind && y.Audit_Type1 == a.Audit_Type1
                    && y.Audit_Type2 == a.Audit_Type2 && y.Brand ==a.Brand &&
                    y.Version == auditType.Where(x=>x.Audit_Kind ==a.Audit_Kind && x.Audit_Type1 == a.Audit_Type1
                    ...
                    Version = auditType.Where(x=>x.Audit_Kind ==a.Audit_Kind && x.Audit_Type1 == a.Audit_Type1
                    && x.Audit_Type2 == a.Audit_Type2 && x.Brand ==a.Brand).OrderByDescending(x=>x.Version).FirstOrDefault().Version
                    };
        ...
        {
            var brand = await _auditTypeMRepository.FindAll(x => x.Audit_Kind.Trim() == "6S").Select(x => x.Brand).Distinct().OrderBy(x => x).ToListAsync();
            return brand;
        }
